"""
Игра «Изгиб Питона».
Реализация классической «Змейки» на Pygame.
"""
...
class GameObject:
    ...
    Содержит позицию и цвет. Является родителем для Apple и Snake.
    """
    ...
    def __init__(self, position=None, body_color=(255, 255, 255)):
        ...
        :param body_color: tuple[int, int, int], RGB-цвет объекта
        """
        if position is None:
...
class Apple(GameObject):
    ...
    def __init__(self):
        """Создаёт яблоко красного цвета и ставит его в случайное место."""
        super().__init__(body_color=(255, 0, 0))
        self.randomize_position()

    def randomize_position(self):
        """Выбирает новую случайную позицию в пределах сетки."""
        self.position = (
...
class Snake(GameObject):
    ...
    def __init__(self):
        """Создаёт змейку длиной 1, направленную вправо, в центре поля."""
    ...
    def update_direction(self):
        """Обновляет направление движения, не позволяя развернуться назад."""
        if self.next_direction is not None:
            new_dx, new_dy = self.next_direction
            curr_dx, curr_dy = self.direction

            if (new_dx, new_dy) != (-curr_dx, -curr_dy):
    ...
    def move(self):
        ...
        1. Считает новую голову.
        2. Проверяет столкновение с собой.
        3. Добавляет голову в список сегментов.
    ...
    def draw(self, surface):
        """
        Отрисовывает змейку и её движение.
...
def handle_keys(snake):
    ...
    Стрелки задают следующее направление движения змейки.
    """
...
def main():
    """Главная функция: инициализация и игровой цикл."""
    pygame.init()
